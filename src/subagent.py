...
def _load_agents_from_dir(directory: Path, agents: dict[str, dict]) -> None:
    ...
    for entry in directory.iterdir():
        ...
        try:
            ...
            perm = meta.get("permission-mode") or meta.get("permission_mode")
...
def get_sub_agent_config(agent_type: str) -> dict:
    """Return {system_prompt, tools, model, permission_mode} for the given agent type.

    ``model`` and ``permission_mode`` are optional overrides sourced from custom
    agent frontmatter (``.claude/agents/*.md``). Built-in types leave them as
    ``None`` so the caller's defaults apply.
    """
    custom = _discover_custom_agents().get(agent_type)
    if custom:
        if custom["allowed_tools"]:
            tools = [
                t for t in tool_definitions if t["name"] in custom["allowed_tools"]
            ]
        else:
        ...
        return {
            ...
            "permission_mode": custom.get("permission_mode"),
        }
