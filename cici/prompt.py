...
from .paths import claude_project_rules_dir, project_rules_dir, scratchpad_dir
...
# Temporary files
IMPORTANT: Always use this session-scoped scratchpad directory for temporary files instead of `/tmp` or other system temp directories:
`{{scratchpad}}`

Use this directory for ALL temporary file needs:
- Intermediate results, working files, or throwaway outputs during multi-step tasks
- One-off scripts, captured command output (see run_shell truncation guidance)
- Any file that would otherwise go to `/tmp` or be scattered across the project

Do NOT write temp files to `/tmp`, to the user's project root, or to arbitrary paths like `/tmp/foo.md`. Only use `/tmp` if the user explicitly requests it. Prefer running throwaway code inline (e.g. `python -c '...'`) over writing a script to disk; if a file is required, put it in the scratchpad above and delete it when no longer needed.
{{git_context}}
{{claude_md}}
{{memory}}
{{skills}}
...
def _load_rules_dir(directory: Path) -> str:
    ...
    for rules_dir in (
        claude_project_rules_dir(directory),
        project_rules_dir(directory),
    ):
...
def build_system_prompt(session_id: str | None = None) -> str:
    ...
    # Resolve a session-scoped scratchpad path and ensure it exists so the
    # model can write there without a preflight `ls`. Fall back to a stable
    # per-user default when no session_id is supplied (e.g. standalone calls).
    scratch = scratchpad_dir(session_id or "default")
    try:
